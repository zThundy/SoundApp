...
export default function RaidAlert({
  iframeRef
}: {
  ...
  const [imageText, setImageText] = useState('${username} has raided with ${viewers} viewers!');
  ...
  async function loadDefaultTemplate() {
    try {
      const res = await window.alerts?.loadTemplate('default-raidAlert');
    ...
    <Stack spacing={2}>
      ...
      <StyledBox>
        <Stack direction="row" flexWrap="wrap" justifyContent={"flex-start"}>
          ...
          <StyledVariable variant="body2" onClick={() => setImageText((prev: any) => prev + '${viewers} ')} style={{ cursor: "pointer" }}>
            <strong>${'{viewers}'}</strong>
            <Tooltip title={t("alert.variableViewers")} placement="top" arrow style={{ cursor: "pointer" }}>
              <Info />
            </Tooltip>
          </StyledVariable>
        </Stack>
