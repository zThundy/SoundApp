...
export default function BitsAlert({
  iframeRef
}: {
  ...
  const [imageText, setImageText] = useState('${username} has cheered ${bits}!');
  ...
  async function loadDefaultTemplate() {
    try {
      const res = await window.alerts?.loadTemplate('default-bitsAlert');
    ...
    <Stack spacing={2}>
      ...
      <StyledBox>
        <Stack direction="row" flexWrap="wrap" justifyContent={"flex-start"}>
          ...
          <StyledVariable variant="body2" onClick={() => setImageText((prev: any) => prev + '${bits} ')} style={{ cursor: "pointer" }}>
            <strong>${'{bits}'}</strong>
            <Tooltip title={t("alert.variableBits")} placement="top" arrow style={{ cursor: "pointer" }}>
              <Info />
            </Tooltip>
          </StyledVariable>
